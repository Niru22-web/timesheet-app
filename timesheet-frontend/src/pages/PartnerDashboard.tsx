...
const PartnerDashboard: React.FC = () => {
  ...
  // Debug logging
  console.log('PartnerDashboard render - User:', user);
  console.log('PartnerDashboard render - User role:', user?.role);

  useEffect(() => {
    console.log('PartnerDashboard useEffect - Fetching data');
    fetchDashboardData();
  }, []);
  ...
  return (
    <div className="space-y-6 animate-fade-in max-h-[calc(100vh-120px)] overflow-hidden flex flex-col">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 flex-none">
        <div>
          <h1 className="text-3xl font-extrabold text-secondary-900 tracking-tight">Partner Dashboard</h1>
          <p className="text-sm font-medium text-secondary-500 mt-1">
            Welcome back, {user?.name || 'Partner'}! Business overview and strategic insights.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1 min-h-0 overflow-hidden">
            ...
            <div className="lg:col-span-8 flex flex-col min-h-0">
              <Card className="flex flex-col h-full overflow-hidden">
                <div className="px-6 py-4 border-b border-secondary-100 flex items-center justify-between bg-white whitespace-nowrap">
                  <h3 className="text-lg font-bold text-secondary-900 tracking-tight">Business Activity Feed</h3>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                  {recentActivities.length > 0 ? (
                    ...
                    <div className="p-10 text-center text-secondary-400 font-bold text-sm">
                      No business activity found. Your team is ready for new projects.
                    </div>
                  )}
                </div>
                <div className="p-4 bg-secondary-50/50 border-t border-secondary-100 text-center flex-none">
                  <p className="text-[10px] font-bold text-secondary-400 uppercase tracking-widest italic tracking-widest">Business insights active <span className="text-success-500 mx-2">●</span> {new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                </div>
              </Card>
            </div>
            ...
            <div className="lg:col-span-4 flex flex-col gap-6 overflow-y-auto no-scrollbar pb-10">
              <Card className="p-6 space-y-6">
                <h3 className="text-lg font-bold text-secondary-900 tracking-tight mb-2">Business Tools</h3>
              ...
              <Card className="p-6 bg-gradient-to-br from-purple-600 to-pink-700 border-none shadow-elevated group relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-4">
                  ...
                  <div className="space-y-1">
                    <h3 className="text-xl font-bold text-white tracking-tight">Partner Portal</h3>
                    <p className="text-sm font-medium text-white/80 leading-relaxed italic opacity-80 pt-1">
                      Strategic business overview with client management and project insights.
                    </p>
                  </div>
                  <Button variant="primary" fullWidth className="bg-white text-purple-600 border-none h-12 text-sm font-bold font-extrabold shadow-none hover:bg-white/90">
