...
const EmployeeDashboard: React.FC = () => {
  ...
  // Debug logging
  console.log('EmployeeDashboard render - User:', user);
  console.log('EmployeeDashboard render - User role:', user?.role);

  useEffect(() => {
    console.log('EmployeeDashboard useEffect - Fetching data');
    fetchDashboardData();
  }, []);
  ...
  const employeeQuickLinks = [
    ...
    { name: 'Documents', color: 'text-green-600', bg: 'bg-green-50', link: '/documents', icon: DocumentTextIcon }
  ];
  ...
  return (
    <div className="space-y-6 animate-fade-in max-h-[calc(100vh-120px)] overflow-hidden flex flex-col">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 flex-none">
        <div>
          <h1 className="text-3xl font-extrabold text-secondary-900 tracking-tight">Employee Dashboard</h1>
          <p className="text-sm font-medium text-secondary-500 mt-1">
            Welcome back, {user?.name || 'Employee'}! Here's your personal work overview.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1 min-h-0 overflow-hidden">
            ...
            <div className="lg:col-span-8 flex flex-col min-h-0">
              <Card className="flex flex-col h-full overflow-hidden">
                <div className="px-6 py-4 border-b border-secondary-100 flex items-center justify-between bg-white whitespace-nowrap">
                  <h3 className="text-lg font-bold text-secondary-900 tracking-tight">My Recent Activity</h3>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                  {recentActivities.length > 0 ? (
                    ...
                    <div className="p-10 text-center text-secondary-400 font-bold text-sm">
                      No activity found yet. Start by logging your timesheet entries.
                    </div>
                  )}
                </div>
                <div className="p-4 bg-secondary-50/50 border-t border-secondary-100 text-center flex-none">
                  <p className="text-[10px] font-bold text-secondary-400 uppercase tracking-widest italic tracking-widest">Personal tracking active <span className="text-success-500 mx-2">●</span> {new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                </div>
              </Card>
            </div>
            ...
            <div className="lg:col-span-4 flex flex-col gap-6 overflow-y-auto no-scrollbar pb-10">
              <Card className="p-6 space-y-6">
                <h3 className="text-lg font-bold text-secondary-900 tracking-tight mb-2">Quick Actions</h3>
              ...
              <Card className="p-6 bg-gradient-to-br from-green-600 to-teal-700 border-none shadow-elevated group relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-4">
                  ...
                  <div className="space-y-1">
                    <h3 className="text-xl font-bold text-white tracking-tight">My Work Portal</h3>
                    <p className="text-sm font-medium text-white/80 leading-relaxed italic opacity-80 pt-1">
                      Track your time, manage claims, and stay updated with your personal work dashboard.
                    </p>
                  </div>
                  <Button variant="primary" fullWidth className="bg-white text-green-600 border-none h-12 text-sm font-bold font-extrabold shadow-none hover:bg-white/90">
                    Log Timesheet Entry
                  </Button>
                </div>
              </Card>
