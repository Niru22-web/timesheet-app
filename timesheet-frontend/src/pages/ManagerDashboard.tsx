...
const ManagerDashboard: React.FC = () => {
  ...
  console.log('ManagerDashboard render - User role:', user?.role);

  useEffect(() => {
    console.log('ManagerDashboard useEffect - Fetching data');
    fetchDashboardData();
  }, []);
  ...
  const managerQuickLinks = [
    ...
    { name: 'Team', color: 'text-indigo-600', bg: 'bg-indigo-50', link: '/employees', icon: UsersIcon },
    { name: 'Projects', color: 'text-cyan-600', bg: 'bg-cyan-50', link: '/projects', icon: BriefcaseIcon },
    { name: 'Clients', color: 'text-purple-600', bg: 'bg-purple-50', link: '/clients', icon: BuildingOfficeIcon },
    { name: 'Reports', color: 'text-success-600', bg: 'bg-success-50', link: '/reports', icon: ChartBarIcon },
  ...
  return (
    <div className="space-y-6 animate-fade-in max-h-[calc(100vh-120px)] overflow-hidden flex flex-col">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 flex-none">
        <div>
          <h1 className="text-3xl font-extrabold text-secondary-900 tracking-tight">Manager Dashboard</h1>
          <p className="text-sm font-medium text-secondary-500 mt-1">
            Welcome back, {user?.name || 'Manager'}! Here's your team overview and project status.
          </p>
        </div>
            ...
            <div className="lg:col-span-8 flex flex-col min-h-0">
              <Card className="flex flex-col h-full overflow-hidden">
                <div className="px-6 py-4 border-b border-secondary-100 flex items-center justify-between bg-white whitespace-nowrap">
                  <h3 className="text-lg font-bold text-secondary-900 tracking-tight">Team Activity Feed</h3>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                  {recentActivities.length > 0 ? (
                    ...
                    <div className="p-10 text-center text-secondary-400 font-bold text-sm">
                      No team activity found. Your team hasn't logged any timesheets yet.
                    </div>
                  )}
                </div>
                <div className="p-4 bg-secondary-50/50 border-t border-secondary-100 text-center flex-none">
                  <p className="text-[10px] font-bold text-secondary-400 uppercase tracking-widest italic tracking-widest">Team sync active <span className="text-success-500 mx-2">●</span> {new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                </div>
              </Card>
            </div>

            {/* Info Grid: Quick Links and Actions */}
            <div className="lg:col-span-4 flex flex-col gap-6 overflow-y-auto no-scrollbar pb-10">
              <Card className="p-6 space-y-6">
                <h3 className="text-lg font-bold text-secondary-900 tracking-tight mb-2">Manager Actions</h3>
              ...
              <Card className="p-6 bg-gradient-to-br from-indigo-600 to-purple-700 border-none shadow-elevated group relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full blur-[80px] -mr-16 -mt-16 pointer-events-none" />
                <div className="relative z-10 space-y-4">
                  <div className="w-10 h-10 bg-white/10 rounded-lg flex items-center justify-center text-white border border-white/20 group-hover:rotate-12 transition-transform duration-500">
                    <UsersIcon className="w-6 h-6" />
                  </div>
                  <div className="space-y-1">
                    <h3 className="text-xl font-bold text-white tracking-tight">Team Management</h3>
                    <p className="text-sm font-medium text-white/80 leading-relaxed italic opacity-80 pt-1">
                      Manage your team members, assign projects, and track performance metrics efficiently.
                    </p>
                  </div>
                  <Button variant="primary" fullWidth className="bg-white text-indigo-600 border-none h-12 text-sm font-bold font-extrabold shadow-none hover:bg-white/90">
                    View Team Details
                  </Button>
                </div>
              </Card>
