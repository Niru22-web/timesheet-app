...
const Timesheet: React.FC = () => {
  ...
  const fetchMasters = async () => {
    try {
      ...
      const [clientsRes, projectsRes, jobsRes] = await Promise.allSettled([
        API.get('/clients'),
        API.get('/projects'),
        API.get('/jobs')
      ]);
      ...
      if (clientsRes.status === 'fulfilled') {
        setClients(clientsRes.value.data?.success ? clientsRes.value.data.data : clientsRes.value.data);
      } else {
      ...
      if (projectsRes.status === 'fulfilled') {
        setProjects(projectsRes.value.data?.success ? projectsRes.value.data.data : projectsRes.value.data);
      } else {
      ...
      if (jobsRes.status === 'fulfilled') {
        setJobs(jobsRes.value.data?.success ? jobsRes.value.data.data : jobsRes.value.data);
      } else {
