...
export class ProjectService {
      ...
      if (user) {
        ...
        if (role === 'partner' || role === 'manager') {
          whereClause.OR = [
            ...
            { users: { some: { employeeId: user.id } } }
          ];
        } else if (role === 'employee') {
          whereClause.users = { some: { employeeId: user.id } };
        }
  ...
  async searchProjects(query: string, filters?: any, user?: any) {
    try {
      let whereClause: any = {};
      ...
      if (query) {
        ...
        if (whereClause.OR) {
          // If we already have an OR for role filtering, we need to AND it with the search OR
          // Prisma doesn't directly support ANDing multiple ORs easily in the top level if one is already present
          // We can use AND: [ { OR: roleFilters }, { OR: searchFilters } ]
