  ...
  try {
    ...
    if (profile && Object.keys(profile).length > 0) {
      ...
      const filteredProfile = {
        ...
        ...(profile.salary && { salary: parseFloat(profile.salary) }),
        ...(profile.seniorityLevel && { seniorityLevel: profile.seniorityLevel }),
        ...(profile.experience && { experience: profile.experience }),
        ...(profile.employmentType && { employmentType: profile.employmentType }),
...
export const getEmployees = async (req: any, res: Response) => {
  try {
    ...
    // Base query filters from request
    ...
    if (search) {
      whereClause.OR = [
        ...
        { officeEmail: { contains: search as string, mode: 'insensitive' } },
  ...
  try {
    ...
    const fullEmployeeDetails = {
      ...
      profile: employee.profile ? {
        ...
        seniorityLevel: employee.profile.seniorityLevel,
      ...
      stats: {
        ...
        timelogsCount: await prisma.timelog.count({ where: { employeeId: id } }),
        teamCount: employee.role === 'Partner' 
          ? await prisma.employee.count({ where: { reportingPartner: id } })
          : employee.role === 'Manager'
            ? await prisma.employee.count({ where: { reportingManager: id } })
            : 0
      },
...
export const toggleEmployeeStatus = async (req: any, res: Response) => {
  try {
    const { id } = req.params;
    ...
    res.json({
      ...
      message: `Employee ${newStatus.toLowerCase()}d successfully`
    });
  } catch (error: any) {
